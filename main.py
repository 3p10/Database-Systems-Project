...
def unsuccessful_payment(cursor):
    try:
        cursor.execute(
            "SELECT Payment.PaymentID, Member.MemberID, Member.Name, Member.Email FROM Payment JOIN Member ON Payment.MemberID = Member.MemberID WHERE Payment.PaymentMethod = 'Unseccessful'")
        unsuccessful_payments = cursor.fetchall()
        print("Unsuccessful payments:")
        for i, payment in enumerate(unsuccessful_payments, 1):
            payment_id, member_id, name, email = payment
            print(f"{i}. PaymentID: {payment_id}, MemberID: {member_id}, Name: {name}, Email: {email}")
    except sqlite3.Error as e:
